	...
	switch ( $value['type'] ) :
		...
		case 'multiselect_categories':
			...
			foreach ($categories_obj as $cat):
				...
				if (is_string($vals) )  {
					...
					if($vals==$cat->cat_ID) $selected=' selected="selected"';
					else $selected='';
				}
				else{
				
					if (in_array($cat->cat_ID, $vals)) $selected=' selected="selected"';
					else $selected='';
				}
		...
		case 'gallery_items':
			...
			if(isset($meta_box_array) && count($meta_box_array)!=0):
				...
				foreach($meta_box_array as $id):
				    ...
				    $i++;
					if($i==1 || $i==count($meta_box_array)) continue;
		...
		case 'select_footer':
			...
			for($i=0;$i<count($footers_array)-1; $i++) {
				...
				if ( $selected_value == $footers_array[$i].','.$custom_footer_layout_array[$i]) { $selected = ' selected="selected"';} 
